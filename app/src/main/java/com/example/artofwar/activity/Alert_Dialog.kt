...
class Alert_Dialog (context: Context){
    ...
    fun showDialog(id:Int)
    {
        ...
        if(id==1){
            ...
            dialog.content.text="로그아웃에 실패하였습니다."
        }
        else if(id==15){
            dialog.content.text="수정을 실패하였습니다.\n잠시후 다시 시도해주세요."
        }
        else if(id==16){
            dialog.content.text="삭제를 실패했습니다.\n잠시후 다시 시도해주세요."
        }
        else if(id==17){
            dialog.content.text="답변전송을 실패했습니다.\n잠시후 다시 시도해주세요."
        }
        else if(id==18){
            dialog.content.text="답변이 달린 이후에는 수정할 수 없습니다."
        }
        else if(id==19){
            dialog.content.text="동영상 파일은 1000MB를 넘을 수 없습니다."
        }
        else if(id==20){
            dialog.content.text="요청을 실패했습니다.\n잠시후 다시 시도해주세요."
        }
        else if(id==21){
            dialog.content.text="요청을 보냈습니다."
        }
